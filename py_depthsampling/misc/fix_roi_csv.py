...
def fix_roi_csv(strCsvRoi, strCsvRoiOut, strVtkIn, varNumHdrRoi=1, strPrcdCoor='POINTS',
                varNumLne=1, varRnd=1):
    """
    ...
    strPrcdCoor : string
        Beginning of string which precedes vertex coordinates in data vtk files.
    varNumLne : int
        Number of lines between string ('strPrcdCoor') and coordinate in vtk file.
    varRnd : int
    ...
    Notes
    -----
    Regions of interest (ROIs) created with paraview on a cortical surface may have
    inconsistent vertex indices with respect to vtk meshes. This may be the case
    if existing ROIs are loaded from disk (loading a paraview 'state' file) and
    then edited. Here, such edited ROIs are loaded,
    """
    ...
    for idxCsv in range(varNumHdrRoi, len(lstCsvRoi)):
        ...
        if (len(lstInt) > 1):
            ...
            if (len(lstInt) > 1):
                ...
                # Error message:
                strErrMsg = ('---Error: More than one vertex from reference VTK '
                             + 'file has the same coordinates as vertex number '
                             + str(idxCsv)
                             + ' from CSV ROI file. There should only be one '
                             + 'vertex per coordinate. Consider reduced rounding '
                             + 'of vertex coordinates.')
                raise ValueError(strErrMsg)

        if (len(lstInt) == 1):
    
            # The list of vertices at intersection of coordinates has length one,
            # containing the index of the vertex (in the vtk mesh) corresponding to
            # the current vertex in the CSV ROI. We replace the (possibly wrong)
            # vertex index in the CSV ROI with the vtk mesh index.
            ...
            # Converte list of strings to string (same as original line from csv
            # file, just with new index):
            strTmp = ','.join(lstTmp)
    ...
    # assign ID = 60076 to respective vertex). If this is not the case,
    # remove this vertex from the ROI. Loop through missing elements in reverse order,
    ...
    for idxErr in list(reversed(lstErr)):
        ...
        if ((int(strTmpPre) + 2) == int(strTmpSce)):
            ...
            print('+++' + strTmpPre + ' ' + lstCsvRoi[idxErr].split(',')[1] + ' ' + strTmpSce)
...
strCsvRoi = '/media/sf_D_DRIVE/MRI_Data_PhD/05_PacMan/20171023/cbs/rh/v2.csv'
strCsvRoiOut = '/media/sf_D_DRIVE/MRI_Data_PhD/05_PacMan/20171023/cbs/rh/v2_mod.csv'
varNumHdrRoi = 1
strVtkIn = '/media/sf_D_DRIVE/MRI_Data_PhD/05_PacMan/20171023/cbs/rh/polar_angle_thr.vtk'
strPrcdCoor ='POINTS'
varNumLne = 1
varRnd = 1

fix_roi_csv(strCsvRoi, strCsvRoiOut, strVtkIn)
