...
def funcParamEccDpthPlt(aryMean,
    ...
    varMin = np.percentile(aryMean, 2.0)
    varMax = np.percentile(aryMean, 98.0)
    ...
    vecClrRngNeg = np.linspace(varMin, 0.0, varNumClr)

    # Lookup vector for positive colour range:
    vecClrRngPos = np.arange(0.0, varMax, varNumClr)
